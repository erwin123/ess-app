import { Component, OnInit, ViewChild, ElementRef, Inject, Input, Output, EventEmitter, AfterViewInit, ChangeDetectorRef } from '@angular/core';
    ...
    if (this.files.length > 0) {
      ...
      async.every(this.files, (each, callback) => {
        if (each !== null && !each.isRealUrl) {
          this.employeeService.postUpload(each, "attach", this.NRP).subscribe(event => {
            let eventRes: any;
