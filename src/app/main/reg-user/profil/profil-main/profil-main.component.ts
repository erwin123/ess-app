...
export class ProfilMainComponent implements OnInit {
  ...
  constructor(private stateService: StateService, private imageCompress: NgxImageCompressService,
    ...
    private _dialog: LyDialog, private route: ActivatedRoute) {
  ...
  fetchParameter(fields, callback) {
    ...
    master.subscribe(res => {
      fields.find(f => f.key === 'AreaCodeKtp').option = res[1].map(m => {
        m.ParentDescription = res[0].find(f => f.AreaCode === m.ParentAreaCode).Description;
        m.text = m.Description + " - " + res[0].find(f => f.AreaCode === m.ParentAreaCode).Description;
        m.value = m.AreaCode;
        return m;
      });
      ...
      fields.find(f => f.key === 'FamilyTax').option = res[2].filter(f => f.Code === 'TAX').map(m => {
        m.text = m.Text;
        m.value = m.Value;
        return m;
      })
  ...
  getQuickCredential(callback) {
    if (this.route.snapshot.paramMap.get('emp')) {
      ...
      this.employeeService.getEmployeeQuickProfile({ Username: emp }, null).subscribe(qc => {
        if (qc.length) {
          callback(qc[0]);
        } else {
          callback(null);
  ...
  bindData(callback) {
    ...
    if (this.route.snapshot.paramMap.get('emp')) {
      ...
      this.employeeService.getEmployee({ NRP: emp }).subscribe(prof => {
        if (prof.length) {
          prof = prof.map(m => {
            m.ClockOut = moment(m.ClockOut).utc().format("YYYY-MM-DD HH:mm");
            m.ClockIn = moment(m.ClockIn).utc().format("YYYY-MM-DD HH:mm");
            return m;
          })
          this.stateService.setBlocking(0);
          callback(prof[0]);
  ...
  readUrlImg(event: any) {
    ...
    if (event.target.files && event.target.files[0]) {
      ...
      reader.onload = (event: any) => {
        ...
        setTimeout(() => {
          this.imageCompress.compressFile(fakeUrl, 1, 50, 50).then(
            result => {
              this.changePP(this.stateService.dataURLtoFile(result, fakeFile.name), this.currentQuickProfile.Username, up => {
                if (up) {
                  let objPut = {
                    Photo: up.filename,
                  ...
                  this.employeeService.putEmployee(objPut).subscribe(put => {
                    if (put.length) {
                      this.deleteFile(this.currentQuickProfile.Username, this.currentQuickProfile.Photo, del => {
                        console.log(del);
                      })
                      this.getQuickCredential((prof) => {
                        if (prof) {
                          if (prof.Username === this.credential.Username) {//same person update localstorage
  ...
  showPopUp(params) {
    ...
    this.accountService.getJSON(params + ".json").subscribe(ed => {
      if (ed.length && this.currentEmployee) {
        ...
        this.stateService.setBlocking(0);
        const dialogRefInfo = this._dialog.open<ProfileItemsComponent>(ProfileItemsComponent, {
          containerClass: this.theme.style(STYLES_DIALOG),
          data: objDialog
        });
        dialogRefInfo.afterClosed.subscribe((res) => {
  ...
  changePP(file, nrp, callback) {
    this.employeeService.postUpload(file, "image", nrp).subscribe(event => {
      let eventRes: any;
      ...
      this.showAlert("<b>Upload foto profil gagal, mohon cek jaringan Anda</b>", true);
    });
  }
