...
@Component({
  selector: "app-set-work-location",
  templateUrl: "./set-work-location.component.html",
  styleUrls: ["./set-work-location.component.scss"]
})
export class SetWorkLocationComponent implements OnInit, AfterViewInit {
  ...
  locationForm = new FormGroup({
    ...
    WorkLocationCode: new FormControl("", [
      Validators.required,
    ...
    WorkLocationPhone: new FormControl("", [
      Validators.required,
      Validators.minLength(8),
      Validators.maxLength(15)
    ]),
    WorkLocationType: new FormControl("", [Validators.required]),
    WorkLocationAddress: new FormControl("", [
      Validators.required,
  ...
  ngAfterViewInit() {
    ...
    this.draw.on("drawend", e => {
      ...
      this.locationForm.controls["Radius"].setValue(
        radius * METERS_PER_UNIT[units]
      );
    });
    //this.map.addControl(this.setupSearch());

    this.route.queryParams.subscribe(params => {
      if (params.mode == 1 && params.id) {
        //view
        this.masterService.getWorkLocation({ Id: params.id }).subscribe(res => {
          let myLoc = res[0];
          this.addPoint([Number(myLoc.Long), Number(myLoc.Lat)]);
          this.addRadius(
            [Number(myLoc.Long), Number(myLoc.Lat)],
            Number(myLoc.Radius)
          );
  ...
  showAlert(msg: string, err: boolean) {
    ...
    dialogRefInfo.afterClosed.subscribe(() => {
      this.router.navigate(["main/admin/maintain-work-location"]);
    });
  }
  onSubmit() {
    if (this.locationForm.valid) {
      if (this.locationForm.controls["Long"].value) {
        ...
        if (this.locationForm.controls["Id"].value > 0) {
          ...
          this.masterService.putWorkLocation(myObj).subscribe(res => {
            this.showAlert("Data berhasil disimpan", false);
          });
        } else {
          this.masterService.postWorkLocation(myObj).subscribe(res => {
            this.showAlert("Data berhasil disimpan", false);
          });
        }
      } else {
  ...
  setupSearch() {
    ...
    geocoder.on("addresschosen", evt => {
      ...
      view.setCenter(
        fromLonLat(transform(evt.coordinate, "EPSG:3857", "EPSG:4326"))
      );
      view.setZoom(16);
    });
    return geocoder;
  ...
  get WorkLocationCode() {
    return this.locationForm.get("WorkLocationCode")!;
  }

  get WorkLocationName() {
    return this.locationForm.get("WorkLocationName")!;
  }

  get WorkLocationPhone() {
    return this.locationForm.get("WorkLocationPhone")!;
  }

  get WorkLocationAddress() {
    return this.locationForm.get("WorkLocationAddress")!;
  }
