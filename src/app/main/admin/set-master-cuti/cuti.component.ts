...
export class SetMasterCutiComponent implements OnInit {
  ...
  fetchField(callback) {
    this.accountService.getJSON("cuti.json").subscribe(res => {
      callback(res);
    });
  }
  ...
  showAlert(msg: string, err: boolean) {
    ...
    dialogRefInfo.afterClosed.subscribe(() => {
      this.router.navigate(['main/admin/maintain-mastercuti']);
    });
  }

  fetchData(id, callback) {
    this.masterService.getCuti({ RowStatus: 1, Id: id }).subscribe(data => {
      if (data.length)
        callback(data[0]);
      else
        callback(null);
  ...
  onSubmit($event) {
    ...
    if ($event.valid) {
      if (this.dataView)
        this.masterService.putCuti($event.getRawValue()).subscribe(upt => {
          this.showAlert("Data Master Tersimpan", false);
        });
      else
        this.masterService.postCuti($event.getRawValue()).subscribe(ins => {
          this.showAlert("Data Master Tersimpan", false);
        });
    }
  }
