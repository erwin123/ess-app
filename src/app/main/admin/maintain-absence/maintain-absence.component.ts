...
export class MaintainAbsenceComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.credential.Role == "ESS_REG") {
      const crit = {
        DirectReportID: this.credential.quickProfile.EmployeeID
      };
      this.fetchData(crit);
    } else {
      this.fetchData({});
  ...
  receiveEvent(data) {
    this.router.navigate(["main/admin/set-absence"], {
      queryParams: { idabs: data.Id }
    });
  }
  ...
  exportData() {
    if (this.tgAwal.isAfter(this.tgAkhir)) {
      ...
      this.absenService
        .getSAPRaw({ Awal: this.tgAwal, Akhir: this.tgAkhir })
        .subscribe(sap => {
          this.excelService.exportAsExcelFile(
            sap,
            "export_raw_" + new Date().toISOString().split("T")[0] + "_"
          );
        });
    } else {
