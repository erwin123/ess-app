...
export class SetAccountComponent implements OnInit {
          ...
          this.route.queryParams.subscribe(params => {
            if (params.emp) {
              ...
              this.employeeService
                .getEmployeeQuickProfile({ Username: params.emp }, null)
                .subscribe(res => {
                  if (res) {
                    ...
                    this.dataView = res.map(m => {
                      return {
                        ...
                        ClockIn: moment(m.ClockIn)
                          .utc()
                          .format("YYYY-MM-DD HH:mm"),
                        ClockOut: moment(m.ClockOut)
                          .utc()
                          .format("YYYY-MM-DD HH:mm"),
                        Role: m.Role,
                        Location: m.AttendanceLocations.map(loc => {
                          return {
  ...
  fetchParameter(fields, callback) {
    let master = forkJoin(
      ...
      this.masterService.getWorkLocation({}),
      this.employeeService.getEmployeeQuickProfileSimple({}),
      this.masterService.getShift()
    );
    master.subscribe(
      res => {
        fields.find(f => f.key === "DivisionID").option = res[0]
          ? res[0].map(m => {
        ...
        fields.find(f => f.key === "DepartmentID").option = res[0]
          ? Departments.map(m => {
              (m.text = m.Name), (m.value = m.Id);
              return m;
            })
          : [];
        //end get
        fields.find(f => f.key === "OrganizationLevelID").option = res[1]
          ? res[1].map(m => {
              (m.text = m.Name), (m.value = m.Id);
              return m;
            })
          : [];
        fields.find(f => f.key === "LocationID").option = res[3]
          ? res[3].map(m => {
              (m.text = m.WorkLocationName), (m.value = Number(m.Id));
              return m;
            })
          : [];
        fields.find(f => f.key === "Location").option = res[2]
          ? res[2].map(m => {
              (m.item_id = m.Id), (m.item_text = m.LocationName);
              return m;
            })
          : [];
        fields.find(f => f.key === "DirectReportID").option = res[3]
          ? res[4]
              .map(m => {
          ...
          : [];
        fields.find(f => f.key === "ShiftID").option = res[5]
          ? res[5].map(m => {
              (m.text = m.ShiftName), (m.value = m.Id);
              return m;
            })
          : [];
      },
  ...
  onSubmit($event) {
    if ($event.valid) {
      ...
      objInsert.AttendanceLocations = objInsert.Location.map(loc => {
        let newLoc = {
          EmployeeID: this.employeeID,
          LocationID: loc.item_id,
          CreateBy: this.credential.Username
        };
        return newLoc;
      });
      objInsert.ClockIn = moment(objInsert.ClockIn).format(
        "YYYY-MM-DD HH:mm:ss"
      );
      objInsert.ClockOut = moment(objInsert.ClockOut).format(
        "YYYY-MM-DD HH:mm:ss"
      );
      if (this.employeeID > 0) {
        objInsert.Id = this.employeeID;
        this.accountService
          .putAccount({ Username: objInsert.Username, Role: objInsert.Role })
          .subscribe();
        delete objInsert.Username;
        delete objInsert.Role;
        this.employeeService.putEmployee(objInsert).subscribe(
          ins => {
            if (ins) {
          ...
          err => {
            ...
            if (err.error.error === "not_unique") {
              this.showAlert(
                "Data tidak tersimpan, NRP sudah terdaftar sebelumnya",
                true
              );
            }
          }
        ...
        this.employeeService.postEmployeeAccount(objInsert).subscribe(
          ins => {
            if (ins) {
              this.showAlert(
                "Data tersimpan, password dikirim ke email karyawan",
                false
              );
            }
          },
