...
export class SetMasterClaimComponent implements OnInit {
  ...
  fetchField(callback) {
    this.accountService.getJSON("claim.json").subscribe(res => {
      callback(res);
    });
  }
  ...
  showAlert(msg: string, err: boolean) {
    ...
    dialogRefInfo.afterClosed.subscribe(() => {
      this.router.navigate(['main/admin/maintain-masterclaim']);
    });
  }

  fetchData(id, callback) {
    this.masterService.getClaim({ RowStatus: 1, Id: id }).subscribe(data => {
      if (data.length)
        callback(data[0]);
      else
        callback(null);
  ...
  onSubmit($event) {
    ...
    if ($event.valid) {
      if (this.dataView)
        this.masterService.putClaim($event.getRawValue()).subscribe(upt => {
          this.showAlert("Data Master Tersimpan", false);
        });
      else
        this.masterService.postClaim($event.getRawValue()).subscribe(ins => {
          this.showAlert("Data Master Tersimpan", false);
        });
    }
  }
