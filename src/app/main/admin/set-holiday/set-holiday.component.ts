...
export class SetHolidayComponent implements OnInit {
  ...
  fetchField(callback) {
    this.accountService.getJSON("holiday.json").subscribe(res => {
      callback(res);
    });
  }
  ...
  showAlert(msg: string, err: boolean) {
    ...
    dialogRefInfo.afterClosed.subscribe(() => {
      this.router.navigate(['main/admin/maintain-holiday']);
    });
  }

  fetchData(id, callback) {
    this.masterService.getHoliday({ RowStatus: 1, Id: id }).subscribe(data => {
      if (data.length)
        callback(data[0]);
      else
        callback(null);
  ...
  onSubmit($event) {
    ...
    if ($event.valid) {
      if (this.dataView)
        this.masterService.putHoliday($event.getRawValue()).subscribe(upt => {
          this.showAlert("Data Master Tersimpan", false);
        });
      else
        this.masterService.postHoliday($event.getRawValue()).subscribe(ins => {
          this.showAlert("Data Master Tersimpan", false);
        });
    }
  }
